...
const mainMenu = (role) => {
  ...
  if (role === 'OWNER') {
    ...
    buttons.push(['♻️ تنشيط كل الأكواد']);
  ...
  try {
    ...
    if (step === 'WAIT_EXPIRE_ALL_CONFIRM') {
      ...
      snap.forEach(child => {
        const d = child.val();
        if (d.expiry > now) {
          updates[`${child.key}/originalExpiry`] = d.expiry;
      ...
      session.step = null;
      return safeReply(ctx,
        `💣 *تم إنهاء وقت ${count} كود نشط!*\n\nكل الأكواد النشطة أصبحت منتهية الآن.\n♻️ تقدر ترجعهم بزرار "تنشيط كل الأكواد"`,
        { parse_mode: 'Markdown', ...mainMenu(role) });
    }

    // ── تنشيط كل الأكواد (رجوع للوقت الأصلي) ──
    if (text === '♻️ تنشيط كل الأكواد') {
      if (role !== 'OWNER') return safeReply(ctx, "❌ للمالك فقط.");
      const snap = await withTimeout(db.ref('students').once('value'));
      const updates = {};
      let count = 0;
      snap.forEach(child => {
        const d = child.val();
        if (d.originalExpiry) {
          updates[`${child.key}/expiry`] = d.originalExpiry;
          updates[`${child.key}/originalExpiry`] = null;
          count++;
        }
      });
      if (count === 0) {
        return safeReply(ctx, "⚠️ مفيش أكواد متوقفة عن طريق زرار الإنهاء.", mainMenu(role));
      }
      await withTimeout(db.ref('students').update(updates));
      return safeReply(ctx,
        `♻️ *تم تنشيط ${count} كود!*\n\nكل الأكواد رجعت لوقتها الأصلي.`,
        { parse_mode: 'Markdown', ...mainMenu(role) });
    }
