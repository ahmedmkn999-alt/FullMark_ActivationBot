...
const CODE_PRICE = 200; // سعر الكود بالجنيه
...
bot.on('text', checkAuth, async (ctx) => {
  ...
  try {
    ...
    // ── 9. أرباحي (موظف أو أونر) ──
    if (text === '💰 أرباحي') {
      const snap = await withTimeout(db.ref('students').once('value'));
      let total = 0;
      snap.forEach(child => {
        const d = child.val();
        if (String(d.creator) === String(userId)) total++;
      });
      const money = total * CODE_PRICE;
      return safeReply(ctx,
        `💰 *أرباحك:*\n\n🔑 الأكواد اللي طلعتها: ${total}\n💵 إجمالي الأرباح: ${money} ج`,
        { parse_mode: 'Markdown' }
      );
    }

    // ── 10. أرباح الموظفين (OWNER فقط) ──
    if (text === '💵 أرباح الموظفين') {
      if (role !== 'OWNER') return safeReply(ctx, "❌ للمالك فقط.");
      ...
      for (const ownerId of OWNERS) {
        const count = creatorMap[String(ownerId)] || 0;
      ...
      msg += `─────────────────\n📊 الإجمالي: ${totalCodes} كود | 💵 ${totalMoney} ج`;

      return safeReply(ctx, msg, { parse_mode: 'Markdown', ...mainMenu(role) });
    }

    // ── 12. إحصائيات عامة (OWNER) ──
